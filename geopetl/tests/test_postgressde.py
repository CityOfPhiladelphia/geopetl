...
def test_all_rows_written(db, user, host, pw,create_test_tables,schema,srid):
    ...
    cur.execute('Select * from {}.{}_{}'.format(schema,point_table_name,srid))
    result = cur.fetchall()
...
def test_assert_data(postgis, csv_data, schema,srid):
    ...
    cur.execute('select objectid,textfield,timestamp,numericfield, timezone, st_astext(shape) as shape, datefield from ' + '{}.{}_{}'.format(schema,point_table_name,srid))
...
def test_dsn_connection(csv_data,db, user, pw, host,postgis, column_definition,schema,create_test_tables,srid):
...
# # # assert DB data with itself
def test_with_types(create_test_tables, db_data,schema, postgis, column_definition, srid):
    # read data from db
    data1 = db_data
    #load to second test table
...
# #
# # # assert data by loading and extracting data without providing schema
def test_without_schema(db_data, postgis, column_definition, csv_data,create_test_tables,schema,srid):
...
#
# #compare csv data with postgres data using geopetl
def test_line_assertion(postgis, csv_data,schema,srid):
    tb = postgis.table('{}.{}_{}'.format(schema,line_table_name,srid))
    rows = etl.fromcsv(line_csv_dir)
    rows.topostgis(postgis.dbo, '{}.{}_{}'.format(schema,line_table_name,srid))
...
#compare csv data with postgres data using geopetl
def test_polygon_assertion(postgis, schema, csv_data, srid):
    tb = postgis.table('{}.{}_{}'.format(schema,polygon_table_name,srid))
    rows = etl.fromcsv(polygon_csv_dir)
    rows.topostgis(postgis.dbo, '{}.{}_{}'.format(schema,polygon_table_name,srid))
